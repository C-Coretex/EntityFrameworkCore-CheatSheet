    ...
    class Program
    {
        ...
        //    Console.WriteLine("Hello World! " + name);
            ...
            using(DataEntity context = new DataEntity())
            {
				...
				//tablesQuery.ForEach(t  => Console.WriteLine((t as Table).Name));
                ...
                //Proper way to sort objects if are possible diferent sorting methods
                //bool isAscending = false;

				//IQueryable<Table> queryTables = (from t in context.Table
				//					        select t);
                //List<Table> tables;
				//if(isAscending)
				//    tables = queryTables.OrderBy(t => t.Name).ToList();
                //else
                //    tables = queryTables.OrderByDescending(t => t.Name).ToList();
